package com.opc.core.service.impl;

import java.util.List;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import com.opc.core.domain.CoreMaterialCategory;
import com.opc.core.mapper.CoreMaterialCategoryMapper;
import com.opc.core.service.ICoreMaterialCategoryService;

/**
 * 素材二级分类Service业务层处理
 *
 * @author opc
 */
@Service
public class CoreMaterialCategoryServiceImpl implements ICoreMaterialCategoryService
{
    @Autowired
    private CoreMaterialCategoryMapper coreMaterialCategoryMapper;

    /**
     * 查询素材二级分类
     *
     * @param id 素材二级分类ID
     * @return 素材二级分类
     */
    @Override
    public CoreMaterialCategory selectCoreMaterialCategoryById(Long id)
    {
        return coreMaterialCategoryMapper.selectCoreMaterialCategoryById(id);
    }

    /**
     * 查询素材二级分类列表
     *
     * @param coreMaterialCategory 素材二级分类
     * @return 素材二级分类
     */
    @Override
    public List<CoreMaterialCategory> selectCoreMaterialCategoryList(CoreMaterialCategory coreMaterialCategory)
    {
        return coreMaterialCategoryMapper.selectCoreMaterialCategoryList(coreMaterialCategory);
    }

    /**
     * 根据套餐分类查询素材二级分类列表
     *
     * @param packageType 套餐分类
     * @return 素材二级分类集合
     */
    @Override
    public List<CoreMaterialCategory> selectCoreMaterialCategoryByPackageType(Integer packageType)
    {
        return coreMaterialCategoryMapper.selectCoreMaterialCategoryByPackageType(packageType);
    }

    /**
     * 查询所有启用的素材二级分类
     *
     * @return 素材二级分类集合
     */
    @Override
    public List<CoreMaterialCategory> selectAllActiveCategory()
    {
        return coreMaterialCategoryMapper.selectAllActiveCategory();
    }

    /**
     * 新增素材二级分类
     *
     * @param coreMaterialCategory 素材二级分类
     * @return 结果
     */
    @Override
    public int insertCoreMaterialCategory(CoreMaterialCategory coreMaterialCategory)
    {
        return coreMaterialCategoryMapper.insertCoreMaterialCategory(coreMaterialCategory);
    }

    /**
     * 修改素材二级分类
     *
     * @param coreMaterialCategory 素材二级分类
     * @return 结果
     */
    @Override
    public int updateCoreMaterialCategory(CoreMaterialCategory coreMaterialCategory)
    {
        return coreMaterialCategoryMapper.updateCoreMaterialCategory(coreMaterialCategory);
    }

    /**
     * 批量删除素材二级分类
     *
     * @param ids 需要删除的素材二级分类ID
     * @return 结果
     */
    @Override
    public int deleteCoreMaterialCategoryByIds(Long[] ids)
    {
        return coreMaterialCategoryMapper.deleteCoreMaterialCategoryByIds(ids);
    }

    /**
     * 删除素材二级分类信息
     *
     * @param id 素材二级分类ID
     * @return 结果
     */
    @Override
    public int deleteCoreMaterialCategoryById(Long id)
    {
        return coreMaterialCategoryMapper.deleteCoreMaterialCategoryById(id);
    }
}
